...
import { Registro, Historial } from '@/types/interfaces';

const AsyncSelect = dynamic(() => import("react-select/async"), { ssr: false });

interface EditCambioModalProps {
  isOpen: boolean;
  onClose: () => void;
  recarga: boolean;
  setRecarga: React.Dispatch<React.SetStateAction<boolean>>;
  cambio?: Historial | null; // El cambio a editar, puede ser null o undefined para crear
}
...
export default function EditCambioModal({
  isOpen,
  onClose,
  recarga,
  setRecarga,
  cambio,
}: EditCambioModalProps) {
  ...
  useEffect(() => {
    if (cambio) {
      setCampoModificado(cambio.campo_modificado || '');
      setValorAnterior(cambio.valor_anterior || '');
      setValorNuevo(cambio.valor_nuevo || '');
      // Buscar el registro asociado
      if (cambio.registro.id) {
        fetch(`/api/registros/show?id=${cambio.registro.id}`)
          .then(res => res.json())
  ...
  const handleSubmit = async () => {
    ...
    const cambioData = {
      ...
      fecha_modificacion: cambio?.fecha_modificacion || new Date().toISOString(),
    };
    ...
    try {
      ...
      if (cambio && cambio.id) {
        // Editar
        response = await fetch(`/api/historial/${cambio.id}`, {
          method: 'PUT',
        ...
        // Crear
        response = await fetch('/api/historial', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(cambioData),
        });
      ...
      if (!response.ok) {
        toast.error(data.message || 'Error al guardar cambio');
        return;
      }
      toast.success(cambio ? 'Cambio editado correctamente' : 'Cambio creado correctamente');
      setRecarga(!recarga);
      setRegistro(null);
      setCampoModificado('');
      setValorAnterior('');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="w-full max-w-xl p-6 bg-white rounded-lg shadow-lg dark:bg-dark-900 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold">{cambio ? 'Editar Cambio' : 'Crear Cambio'}</h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              ...
              <AsyncSelect
                ...
                isDisabled={!!cambio} // No permitir cambiar el registro en edición
              />
            </div>
            <div className="col-span-2">
          ...
          <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200 dark:border-gray-700">
            ...
            >
              {cambio ? 'Guardar Cambios' : 'Crear Cambio'}
            </Button>
          </div>
        </div>
      </div>
