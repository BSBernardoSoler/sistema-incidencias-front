...
export default function SelectActions({
  ...
  const deleteUser = async (observacionId: number) => {
    const res = await fetch(`/api/observaciones?id=${observacionId}`, {
      method: "DELETE",
    });

    if (!res.ok) {
      const errorData = await res.json();
      toast.error(errorData.message || "Error deleting observación");
      return;
    } else {
      toast.success("Observación deleted successfully");
      setRecarga(!recarga);
    }
  };
